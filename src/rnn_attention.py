...
class AttentionModel:
    def __init__(self, opts, sess, MAXLEN, vocab, batch_size = 1000):
        self.dim = 300
        self.sess = sess
    ...
    def build_model(self):
        ...
        tmp2 = tf.matmul(tf.reshape(self.h_n_repeat, shape = [self.batch_size * self.MAXLEN, self.h_dim]), self.W_h)
        self.Whn = tf.reshape(tmp2, shape = [self.batch_size, self.MAXLEN, self.h_dim], name = "Whn");
        self.M = tf.tanh(tf.add(self.Wy, self.Whn), name = "M")
        # print "M",self.M

        # use attention
        self.W_att = tf.get_variable("W_att", shape = [self.h_dim, 1])  # h x 1
        tmp3 = tf.matmul(tf.reshape(self.M, shape = [self.batch_size * self.MAXLEN, self.h_dim]), self.W_att)
        # need 1 here so that later can do multiplication with h x L
        self.att = tf.nn.softmax(tf.reshape(tmp3, shape = [self.batch_size, 1, self.MAXLEN], name = "att"))  # nb x 1 x Xmax
        # print "att",self.att
...
#     vocab = get_vocab(train)
    ...
    with tf.Session() as sess:
        model = AttentionModel(options, sess, MAXLEN, vocab_dict, batch_size = 1)
        model.build_model()
